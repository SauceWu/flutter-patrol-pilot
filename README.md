...
```bash
git clone <this-repo-url> ~/.claude/skills/flutter-patrol-pilot
```
...
| 7 | `project.pbxproj` via xcodeproj gem:`objectVersion 70 → 60`,`ENABLE_USER_SCRIPT_SANDBOXING YES → NO`(全部 build configs) | Issue 13, 14 |
...
4. Xcode 26 特殊:用 xcodeproj gem(推荐,跟 `init_project.sh` Step 7 同款):

   ```bash
   ruby -r xcodeproj <<'RUBY'
   p = Xcodeproj::Project.open('ios/Runner.xcodeproj')
   p.instance_variable_set(:@object_version, '60') if p.object_version.to_i > 60
   (p.root_object.build_configurations + p.targets.flat_map(&:build_configurations)).each do |c|
     c.build_settings['ENABLE_USER_SCRIPT_SANDBOXING'] = 'NO' if c.build_settings['ENABLE_USER_SCRIPT_SANDBOXING'] == 'YES'
   end
   p.save
   RUBY
   ```

   或一行 sed(脆弱,但临时调试可用):`sed -i '' 's/ENABLE_USER_SCRIPT_SANDBOXING = YES/ENABLE_USER_SCRIPT_SANDBOXING = NO/g; s/objectVersion = 70;/objectVersion = 60;/' ios/Runner.xcodeproj/project.pbxproj`
...
## 可选: AXe CLI(让 a11y triage 更省 token)

[`AXe`](https://github.com/cameroncooke/AXe) 是 `sim_snapshot.sh --tree` 的可选依赖 —— 用来从 iOS 模拟器导出**结构化的可访问性树**(每个控件的 label / role / value / enabled / focused),通常**比一张截图便宜 ~10× tokens**。装它纯粹是省钱,不装也能用,只是 fallback 到截图。

**安装(需要 brew):**

```bash
bash <skill>/scripts/install_axe.sh         # 幂等;已装就 no-op
bash <skill>/scripts/install_axe.sh --dry-run   # 预览不执行
bash <skill>/scripts/install_axe.sh --force     # 重装
```

底层就一行 `brew install cameroncooke/axe/axe`。stdout 一行 JSON(`success` / `action: "noop" | "installed" | "dry_run" | "failed"` / `axe_path` / `axe_version` / `error`),适合 agent 消费。

**关键契约:agent 不会擅自装。**`init_project.sh` 完成后,JSON summary 里有 `axe_present: bool`;为 false 时 `next_steps[]` 里会附一条 `OPTIONAL: ...` 提示。当 agent 因为 5-E 类失败需要调 `sim_snapshot.sh --tree` 而又发现 axe 缺失,它**会先问你**"要不要装",你点头才跑 `install_axe.sh`。详见 `SKILL.md` §"Optional: AXe CLI"。

不装的代价:`sim_snapshot.sh --tree` 自动降级到 `xcrun simctl io screenshot`,JSON 里 `warning` 字段会标 `axe not installed — fell back to screenshot`,迭代依然能继续。
