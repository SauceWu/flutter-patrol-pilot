...
| `Module 'patrol' not found` (in `RunnerUITests.m` compile error) | 5-A (testing_infra / Podfile setup) | `RunnerUITests` target exists in Xcode but `Podfile` has no matching `target 'RunnerUITests' do ... end` block — so `pod install` never generated `Pods-Runner-RunnerUITests.xcconfig`. Fix: add `target 'RunnerUITests' do; inherit! :complete; end` **inside** `target 'Runner' do` block, also add `use_modular_headers!`. Re-run `pod install`. See troubleshooting.md Issue 12. Do NOT try `Cmd+B` in Xcode GUI — `CLEAR_PERMISSIONS`/`FULL_ISOLATION` macros are only injected by `patrol build`/`patrol test` | Change Dart code |
| `Sandbox: ... deny(1) file-write-create ... RunnerUITests.xctest` / `mkdir: ... Operation not permitted` | 5-A (build config / Xcode 26+ sandbox) | Xcode 26 sets `ENABLE_USER_SCRIPT_SANDBOXING = YES` by default on newly-added targets. Flutter's `Thin Binary` / `xcode_backend.sh` run scripts can't write into `build/`. Fix: `sed -i '' 's/ENABLE_USER_SCRIPT_SANDBOXING = YES;/ENABLE_USER_SCRIPT_SANDBOXING = NO;/g' ios/Runner.xcodeproj/project.pbxproj`. See troubleshooting.md Issue 14 | Retry build without changing sandbox setting |
...
| `[Xcodeproj] Unable to find compatibility version string for object version '70'` (pod install) | 5-D (Xcode 26+ / CocoaPods mismatch) | Xcode 26 writes `objectVersion = 70`; CocoaPods 1.16.x's `xcodeproj` gem only understands up to 63. Fix: `sed -i '' 's/objectVersion = 70;/objectVersion = 60;/' ios/Runner.xcodeproj/project.pbxproj` then re-run `pod install`. See troubleshooting.md Issue 13 | Wait for CocoaPods stable release |
| `Clone 1` / `Clone 2` / `Clone 3` sims appear during a test run; multiple Simulator windows open | 5-D (xcode_parallel_testing) | Xcode's `xcodebuild test-without-building` defaults to `-parallel-testing-enabled YES`, which CLONES the target sim to parallelize tests. The Runner scheme also marks RunnerUITests as `parallelizable="YES"`. patrol_cli 4.3.x does not override either. Fix: use the skill's direct path (`scripts/run_test.sh` since v0.3 invokes `xcodebuild test-without-building … -parallel-testing-enabled NO -disable-concurrent-destination-testing -destination id=<UDID>`) and/or flip `parallelizable = "YES"` → `"NO"` in `ios/Runner.xcodeproj/xcshareddata/xcschemes/Runner.xcscheme`. Immediate recovery: `pkill -9 -f xcodebuild; xcrun simctl shutdown all; killall Simulator`. See troubleshooting.md Issue 15 | Retry with `patrol test` / legacy path (will clone again) |
| `The test runner timed out while preparing to run tests.` AND run elapsed ≈360s AND `test.log` shows XCUI stuck at `Wait for <bundle> to idle` (never printed `PatrolAppServiceClient: connected` / never reached Flutter code) | 5-A (testing_infra / Xcode 26 Swift Testing deps missing) | Runner.app dyld-crashes on launch because Xcode 26 adds a new Swift Testing runtime that needs `_Testing_Foundation.framework` + `_Testing_CoreGraphics` + `_Testing_CoreImage` + `_Testing_UIKit` + `lib_TestingInterop.dylib`, and Flutter's build doesn't embed them. XCUI waits for the app to reach idle; the app never starts; 6-minute timeout. Confirm by manually launching the app with `xcrun simctl launch --console-pty <UDID> <bundle_id>` and looking for `dyld[...]: Library not loaded: @rpath/_Testing_Foundation.framework/...`. Fix: `scripts/build.sh` v0.3+ auto-injects these files into both `Runner.app/Frameworks/` and `RunnerUITests-Runner.app/Frameworks/` before `xcrun simctl install` — re-run `build.sh` and verify the `[build.sh] injected ... → Runner.app` log lines appear. See troubleshooting.md Issue 16 | Change Dart code / retry without re-injecting frameworks |
...
- `[Xcodeproj] Unable to find compatibility version string for object version '70'` — Xcode 26 + CocoaPods 1.16.x mismatch (see troubleshooting.md Issue 13)
- **xcode_parallel_testing** — `Clone 1/2/3` simulators appear during a run; multiple Simulator windows spawn — Xcode's default `-parallel-testing-enabled YES` makes `xcodebuild test-without-building` clone the target sim. Fix by letting `scripts/run_test.sh` drive xcodebuild directly (it disables parallel testing + uses `-destination id=<UDID>`) and/or by setting `parallelizable = "NO"` in `Runner.xcscheme`. See troubleshooting.md Issue 15
- **xcode26_swift_testing_deps_missing** — "The test runner timed out while preparing to run tests." with 6-minute elapsed and no Flutter/Dart output in the test log — Runner.app dyld-crashes on launch because Xcode 26 adds Swift Testing runtime deps (`_Testing_Foundation.framework`, `lib_TestingInterop.dylib`, etc.) that Flutter doesn't embed. XCUI's `Wait for <bundle> to idle` times out because the app never starts. Fix by letting `scripts/build.sh` v0.3+ auto-inject the missing files into both app bundles before `simctl install`. See troubleshooting.md Issue 16
...
- Downgrade `ios/Runner.xcodeproj/project.pbxproj` `objectVersion` 70 → 60 if CocoaPods refuses to parse (Xcode 26 compatibility workaround)
...
- Shut down polluted sims: `xcrun simctl shutdown all && killall Simulator 2>/dev/null || true` — then rerun with `run_test.sh --isolate-sim`
